    ...
    public class DGELQF
    {
                         ...
                         , int LWORK, ref int INFO)
        {
            ...
            if (NB > 1 && NB < K)
            {
                ...
                // *
                NX = Math.Max(0, this._ilaenv.Run(3, "DGELQF", " ", M, N,  - 1,  - 1));
                if (NX < K)
                {
                    // *
                    ...
                    if (LWORK < IWS)
                    {
                        ...
                        // *
                        NB = LWORK / LDWORK;
                        NBMIN = Math.Max(2, this._ilaenv.Run(2, "DGELQF", " ", M, N,  - 1,  - 1));
                    }
                }
            }
            // *
            if (NB >= NBMIN && NB < K && NX < K)
            {
                ...
                for (I = 1; (NB >= 0) ? (I <= K - NX) : (I >= K - NX); I += NB)
                {
                    ...
                    // *           A(i:i+ib-1,i:n)
                    // *
                    this._dgelq2.Run(IB, N - I + 1, ref A, I+I * LDA + o_a, LDA, ref TAU, I + o_tau, ref WORK, offset_work
                                     , ref IINFO);
                    if (I + IB <= M)
                    {
                        // *
                        ...
                        // *
                        this._dlarfb.Run("Right", "No transpose", "Forward", "Rowwise", M - I - IB + 1, N - I + 1
            ...
            if (I <= K)
            {
                this._dgelq2.Run(M - I + 1, N - I + 1, ref A, I+I * LDA + o_a, LDA, ref TAU, I + o_tau, ref WORK, offset_work
                                 , ref IINFO);
            }
            // *
            WORK[1 + o_work] = IWS;
