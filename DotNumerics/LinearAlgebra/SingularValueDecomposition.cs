...
namespace DotNumerics.LinearAlgebra
{
    ...
    public sealed class SingularValueDecomposition
    {
        ...
        public void ComputeSVD(Matrix A, out Matrix S, out Matrix U, out Matrix VT)
        {
            if (ARMADILO.Enabled)
            {
                ARMADILO.svd(A, out S, out U, out VT);
            }
        ...
        {
            if (ARMADILO.Enabled) return ARMADILO.pinv(A);

            var svd = new SingularValueDecomposition();
            Matrix S, U, VT;
            svd.ComputeSVD(A, out S, out U, out VT);
            var I = Math.Min(S.RowCount, S.ColumnCount);
            for (int i = 0; i < I; i++)
                if (Math.Abs(S[i, i]) > 1 - 15)
                    S[i, i] = 1.0 / S[i, i];
            return VT.Transpose() * S.Transpose() * U.Transpose();
        }
        public static ComplexMatrix PseudoInverse(ComplexMatrix A)
        {
            if (ARMADILO.Enabled)
                return ARMADILO.pinv(A);
            else
                throw new NotImplementedException("ComplexMatrix.PseudoInverse: only with armadillo");
        }
